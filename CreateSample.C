...
int main(int argc,char** argv)
{
  ...
  if (runmode>0) { 
    ...
    for(int i=0;i<sigNo;i++){
      //initial
	  //
	  //smear invariance mass
      //msig=gRandom->Gaus(mpeak,sigma);
      msig = mpeak;

	  cmspsig = gRandom->Gaus(cmsp,0.2*cmsp);
	  ...
	  // smear momentum in center of mass system
	  // cmspsig1 = gRandom->Gaus(cmspsig1, 0.005*cmspsig1);
	  // cmspsig2 = gRandom->Gaus(cmspsig2, 0.005*cmspsig2);
      ...
      double axiscosangle = gRandom->Uniform(-1,1);
      double axisangle = acos(axiscosangle);
      //double axisangle = gRandom->Uniform(0,pi);
      //axisangle = gRandom->Uniform(0,twopi);
      p1_1[0] = p1[0];
      p1_1[1] = p1[1]*cos(axisangle)+p1[2]*sin(axisangle);
      p1_1[2] = p1[2]*cos(axisangle)-p1[1]*sin(axisangle);
      p2_1[0] = p2[0];
      p2_1[1] = p2[1]*cos(axisangle)+p2[2]*sin(axisangle);
      p2_1[2] = p2[2]*cos(axisangle)-p2[1]*sin(axisangle);

      // rotate phi
	  double p1_2[3],p2_2[3];
      axisangle = gRandom->Uniform(0,twopi);
      p1_2[0] = p1_1[0]*cos(axisangle)+p1_1[1]*sin(axisangle);
      p1_2[1] = p1_1[1]*cos(axisangle)-p1_1[0]*sin(axisangle);
      p1_2[2] = p1_1[2];
      p2_2[0] = p2_1[0]*cos(axisangle)+p2_1[1]*sin(axisangle);
      p2_2[1] = p2_1[1]*cos(axisangle)-p2_1[0]*sin(axisangle);
      p2_2[2] = p2_1[2];
